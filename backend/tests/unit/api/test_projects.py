...
def make_client(users: list[User], projects: list[Project]):
    uow = make_uow(
        ...
        project_repository=InMemoryProjectRepository(projects=projects),
    )
    return (
...
def client(admin, labeler):
    uow_ctx, test_client = make_client(users=[admin, labeler], projects=[])
    with uow_ctx:
        yield test_client
...
def client_with_project(admin, labeler, project):
    uow_ctx, test_client = make_client(users=[admin, labeler], projects=[project])
    with uow_ctx:
        yield test_client
...
def test_list_projects_returns_labeler_count(admin, labeler):
    ...
    uow_ctx, test_client = make_client(users=[admin, labeler], projects=[p])
    with uow_ctx:
...
def test_list_projects_returns_all_projects(admin):
    ...
    uow_ctx, test_client = make_client(users=[admin], projects=[p1, p2])
    with uow_ctx:
        response = test_client.get("/v1/projects")
    assert response.status_code == 200
    body = response.json()
...
def test_list_projects_pagination(admin):
    ...
    uow_ctx, test_client = make_client(users=[admin], projects=projects)
    with uow_ctx:
...
def test_list_projects_filter_by_name(admin):
    ...
    uow_ctx, test_client = make_client(
        users=[admin],
        projects=[p1, p2, p3],
    )
    with uow_ctx:
...
def test_list_projects_filter_by_name_case_insensitive(admin):
    ...
    uow_ctx, test_client = make_client(users=[admin], projects=[p1])
    with uow_ctx:
...
def test_list_projects_filter_by_name_contains(admin):
    ...
    uow_ctx, test_client = make_client(users=[admin], projects=[p1, p2, p3])
    with uow_ctx:
