...
async def test_upload_image_returns_image_with_correct_project(use_case, project):
    image = await use_case.execute(project_id=project.id, data=b"fake-image", content_type="image/jpeg")
...
async def test_upload_image_raises_when_project_not_found(use_case):
    with pytest.raises(ProjectNotFound):
        await use_case.execute(project_id=uuid4(), data=b"fake-image", content_type="image/jpeg")
