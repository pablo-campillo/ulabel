...
def project_without_labeler(admin):
    return Project.create(id=uuid4(), owner=admin, name="Project B", description="desc", labels={"dog"})
...
async def test_does_not_return_projects_labeler_is_not_in(use_case, labeler, project_without_labeler):
    result = await use_case.execute(labeler_id=labeler.id)
