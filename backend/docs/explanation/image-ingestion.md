...
3. A **presigned URL** is generated for the image, with an expiry configured via `tasks.image_assignment_timeout_seconds` in `config.yml` (default: 60 seconds).
...
- **Assignment URLs** (expiry configured via `config.yml`, default 60 s): Allow labelers to view the image in the browser without direct storage credentials.
